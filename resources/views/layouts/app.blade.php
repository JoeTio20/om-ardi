...
<header class="sticky top-0 z-50 bg-cream border-b border-gray-200">
  <div class="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
    ...
    <div class="flex items-center gap-4">
      <button class="md:hidden flex flex-col gap-1.5 p-1"
    ...
    <div class="flex items-center gap-3">
      <a href=" route('lang.switch', app()->getLocale() === 'en' ? 'id' : 'en') "
         class="hidden sm:flex items-center gap-1 text-[11px] font-semibold tracking-widest border border-gray-300 rounded-full px-3 py-1 hover:bg-charcoal hover:text-cream transition">
  ...
  <div id="mobile-menu" class="md:hidden bg-cream border-t border-gray-100 px-6 py-5">
    <nav class="flex flex-col gap-5">
      <a href=" route('home') " class="text-sm tracking-widest uppercase text-charcoal"
         onclick="document.getElementById('mobile-menu').classList.remove('open')"> __('messages.nav_home') </a>
      <a href=" route('philosophy') " class="text-sm tracking-widest uppercase text-charcoal"
         onclick="document.getElementById('mobile-menu').classList.remove('open')"> __('messages.nav_philosophy') </a>
      <a href=" route('product') " class="text-sm tracking-widest uppercase text-charcoal"
         onclick="document.getElementById('mobile-menu').classList.remove('open')"> __('messages.nav_product') </a>
      <hr class="border-gray-200">
      <a href=" route('lang.switch', app()->getLocale() === 'en' ? 'id' : 'en') "
         class="text-sm text-warm-gray"> app()->getLocale() === 'en' ? 'Switch to ID' : 'Switch to EN' </a>
    </nav>
  </div>
</header>
  ...
  <div class="max-w-7xl mx-auto px-4 md:px-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center pt-5 text-xs text-white/30 gap-2">
      ...
      <div class="flex gap-5">
        <span> __('messages.f_global') </span>
        <span> __('messages.f_warranty') </span>
      </div>
    </div>
  </div>
</footer>
...
// Add to cart via AJAX (for .btn-add-cart buttons on home/product pages)
