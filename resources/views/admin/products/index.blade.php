...
<div class="px-5 pt-6 pb-3 flex items-center justify-between">
  <div>
    ...
    <p class="text-[10px] tracking-widest uppercase text-[#94A3B8] font-semibold">Admin Panel</p>
  </div>
...
<div id="srch" class="hidden px-5 pb-3">
  <input type="text" placeholder="Cari nama produk..." class="w-full bg-white border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none shadow-sm focus:border-[#0D1F3C]" oninput="filterCards(this.value)">
</div>

@if(session('success'))
<div class="mx-5 mb-3 flex items-center gap-2 bg-green-50 border border-green-200 text-green-700 text-sm px-4 py-3 rounded-xl">
...
@php
  ...
  $avgPrice = $products->where('is_active', true)->avg('price');
@endphp

<!-- STATS BANNER -->
<div class="mx-5 mb-5 rounded-2xl overflow-hidden" style="background:#0D1F3C">
  <div class="relative p-5">
    <p class="text-[10px] tracking-widest font-bold uppercase mb-1" style="color:rgba(255,255,255,.5)">AVERAGE PRICE</p>
    <p class="text-3xl font-bold text-white mb-2">Rp {{ number_format($avgPrice ?? 0,0,',','.') }}</p>
    ...
    <div class="grid grid-cols-2 gap-3 pt-4" style="border-top:1px solid rgba(255,255,255,.1)">
      <div>
        ...
        <p class="text-base font-bold text-white">{{ $products->count() }} Items</p>
      </div>
      <div>
        <p class="text-[9px] tracking-widest font-bold uppercase mb-1" style="color:rgba(255,255,255,.4)">AKTIF</p>
        <p class="text-base font-bold text-white">{{ $activeCount }} Produk</p>
      </div>
    </div>
  </div>
</div>
...
<div class="px-5 pb-28">
  ...
  <div class="space-y-3" id="product-list">
    ...
    @php
      $img = is_array($p->images) ? ($p->images[0] ?? '/IMAGE/SUPER.jpeg') : '/IMAGE/SUPER.jpeg';
    @endphp
    <div class="card product-item" data-name="{{ strtolower($p->name) }}">
      <div class="flex gap-4 items-start mb-3">
        <div class="w-16 h-16 rounded-xl overflow-hidden bg-[#F1F5F9] flex-shrink-0">
          <img src="{{ $img }}" alt="{{ $p->name }}" class="w-full h-full object-cover" onerror="this.src='/IMAGE/SUPER.jpeg'">
        </div>
        <div class="flex-1 min-w-0">
          <div class="flex justify-between items-start gap-2 mb-1">
            <p class="serif text-base font-bold text-[#0D1F3C] leading-tight">{{ $p->name }}</p>
            <span class="text-[9px] font-bold tracking-widest uppercase px-2 py-0.5 rounded flex-shrink-0 {{ $p->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-500' }}">
              {{ $p->is_active ? 'Aktif' : 'Nonaktif' }}
            </span>
          </div>
          <p class="text-xs text-[#94A3B8] mb-1">{{ Str::limit($p->description ?? '', 60) }}</p>
    ...
    <div class="text-center py-16 text-gray-400">
      ...
      <a href="{{ route('admin.products.create') }}" class="text-[#0D1F3C] font-semibold underline text-sm">Tambah produk pertama</a>
    </div>
    @endforelse
  </div>
</div>
