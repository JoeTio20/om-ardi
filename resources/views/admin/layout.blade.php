...
<p class="serif font-bold text-[#2C1810] text-base">Admin Panel</p>
<div class="w-8 h-8 rounded-full bg-[#6B5B4E] flex items-center justify-center text-white text-xs font-semibold">{{ strtoupper(substr(optional(auth('admin')->user())->name ?? 'A', 0, 1)) }}</div>
</div>
...
<p class="serif text-[17px] font-bold text-[#2C1810]">Halo, {{ optional(auth('admin')->user())->name ?? 'Admin' }}</p>
</div>
<!-- Close mobile -->
...
<div class="w-8 h-8 rounded-full bg-[#6B5B4E] flex items-center justify-center text-white text-xs font-semibold flex-shrink-0">{{ strtoupper(substr(optional(auth('admin')->user())->name ?? 'A', 0, 1)) }}</div>
<div class="min-w-0">
<p class="text-[12.5px] font-medium text-[#2C1810] truncate">{{ optional(auth('admin')->user())->name ?? 'Admin' }}</p>
<form method="POST" action="{{ route('admin.logout') }}">
@csrf
<button type="submit" class="text-[10px] text-[#A08070] uppercase tracking-wide bg-transparent border-none cursor-pointer p-0 hover:text-[#3D2B1F] transition">Logout</button>
</form>
</div></div>
</div>
</aside>
